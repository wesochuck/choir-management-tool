...
test('exportToCSV duplicates section leaders into dedicated section preserving order', () => {
  ...
  const janeCount =
    csv.split('"Jane Singer","jane@example.com","555-123-4567","S1","Active"').length - 1;
  const alexCount =
    csv.split('"Alex ""Ace"", Singer","alex@example.com","","T2","Active"').length - 1;
...
test('profileService.updateProfile clear email - save-order sequence', async (t) => {
  ...
  try {
    const { profileService } = await import('../src/services/profileService.ts');
    const updated = await profileService.updateProfile('profile123', {
      name: 'John Doe',
      email: '',
    });
...
test('profileService.getDirectoryProfiles queries with active directory filters', async (t) => {
  const originalCollection = pb.collection;
  const mockGetFullList = t.mock.fn(async (options?: Record<string, unknown>) => {
    return [
      { id: 'p1', name: 'Alice', globalStatus: 'Active', voicePart: 'S1', showInDirectory: true },
    ];
  });
  ...
  try {
    const { profileService } = await import('../src/services/profileService.ts');
    const result = await profileService.getDirectoryProfiles();

    assert.equal(result.length, 1);
    assert.equal(result[0].id, 'p1');
    assert.equal(mockGetFullList.mock.callCount(), 1);

    const callArgs = mockGetFullList.mock.calls[0].arguments[0] as Record<string, unknown>;
    assert.equal(callArgs.filter, 'globalStatus != "Inactive" && showInDirectory != false');
    assert.equal(callArgs.sort, 'name');
    assert.equal(callArgs.expand, 'user');
  } finally {
    pb.collection = originalCollection;
  }
});
