...
import {
  shouldQueueMessage,
  enqueueBulkMessage,
} from '../../pocketbase/pb_hooks_src/email/messageHookRules';
import type {
  PocketBaseRecord,
  PocketBaseApp,
} from '../../pocketbase/pb_hooks_src/email/emailTypes';
...
test('shouldQueueMessage validation and transitions', () => {
  ...
  assert.strictEqual(
    shouldQueueMessage(createRecord('Draft', 'Email')),
    false,
    'Draft should not queue'
  );
  assert.strictEqual(
    shouldQueueMessage(createRecord('Sent', 'SMS')),
    true,
    'SMS only should queue'
  );
  assert.strictEqual(
    shouldQueueMessage(createRecord('Sent', 'Email')),
    true,
    'Sent Email should queue'
  );
  assert.strictEqual(
    shouldQueueMessage(createRecord('Sent', 'Both')),
    true,
    'Sent Both should queue'
  );
  assert.strictEqual(
    shouldQueueMessage(createRecord('Failed', 'Email')),
    false,
    'Failed Email should NOT queue automatically'
  );
  assert.strictEqual(
    shouldQueueMessage(createRecord('Archived', 'Email')),
    false,
    'Archived Email should NOT queue'
  );

  // Test transition logic (create vs update checks)
  assert.strictEqual(
    shouldQueueMessage(createRecord('Sent', 'Email'), 'Draft'),
    true,
    'Draft to Sent is allowed'
  );
  assert.strictEqual(
    shouldQueueMessage(createRecord('Sent', 'Email'), 'Failed'),
    true,
    'Failed to Sent is allowed (resend)'
  );
  assert.strictEqual(
    shouldQueueMessage(createRecord('Sent', 'Email'), 'Sent'),
    false,
    'Sent to Sent is blocked (duplicate protection)'
  );
  assert.strictEqual(
    shouldQueueMessage(createRecord('Draft', 'Email'), 'Draft'),
    false,
    'Draft to Draft is blocked'
  );
});
...
test('enqueueBulkMessage creates SMS entries for type=SMS', () => {
  ...
  assert.strictEqual(
    savedRecords.length,
    2,
    'SMS type creates 2 queue records (2 valid phones, 1 too short, 1 missing)'
  );

  const rec1 = savedRecords[0];
  assert.strictEqual(rec1.get('recipientId'), 'usr-1');
  assert.strictEqual(rec1.get('recipientEmail'), '5551234567');
  assert.strictEqual(rec1.get('recipientName'), 'John Doe');
  ...
  assert.strictEqual(rec2.get('recipientEmail'), '2125558901');
  assert.strictEqual(rec2.get('recipientName'), 'Performer');
...
test('enqueueBulkMessage creates both SMS and email entries for type=Both', () => {
  ...
  assert.strictEqual(smsRecords[0].get('recipientEmail'), '5551234567');
