...
// Generated on: 2026-05-23T05:21:42.177Z
...
function parseSignedToken(token, requiredKeys) {
    ...
    const { eventId, profileIds } = data;
    if (!eventId || !profileIds || !Array.isArray(profileIds)) return e.json(400, { error: "Invalid data" });
    const secret = getHmacSecret();
    if (!secret) return e.json(500, { error: "HMAC_SECRET not configured" });
    const tokens = {};
...
function parseSignedToken(token, requiredKeys) {
    ...
    if (!token) return e.json(400, { error: "Missing token" });
    const parts = parseSignedToken(token, ["e", "p", "s"]);
    if (!parts) return e.json(400, { error: "Invalid token format" });
    const secret = getHmacSecret();
    const payload = `e=${parts.e}&p=${parts.p}`;
    if (!$security.equal(parts.s, $security.hs256(payload, secret))) {
    ...
    try {
        ...
        if (!event.get("isOpenForRSVP")) return e.json(410, { error: "RSVP window closed" });
        ...
        if (event.get("type") === "Performance") {
            try {
                ...
                list.forEach(reh => {
                    let rVenueName = "TBD";
                    try { const vId = reh.get("venue"); if (vId) rVenueName = $app.findRecordById("venues", vId).get("name"); } catch (vErr) {}
                    rehearsals.push({ id: reh.id, title: reh.get("title") || "", type: reh.get("type") || "", date: reh.get("date") || "", details: reh.get("details") || "", expand: { venue: { name: rVenueName } } });
                });
            } catch (rehErr) {}
        }
        let currentRsvp = "Pending";
        try { currentRsvp = $app.findFirstRecordByFilter("eventRosters", "event = {:e} && profile = {:p}", { e: parts.e, p: parts.p }).get("rsvp") || "Pending"; } catch (rosterErr) {}
        let venueName = "TBD";
        try { const vId = event.get("venue"); if (vId) venueName = $app.findRecordById("venues", vId).get("name"); } catch (e) {}
        return e.json(200, { event: { id: event.id, title: event.get("title") || "", type: event.get("type") || "", date: event.get("date") || "", details: event.get("details") || "", expand: { venue: { name: venueName } } }, profile: { id: profile.id, name: profile.get("name") || "", voicePart: profile.get("voicePart") || "" }, currentRsvp, rehearsals });
    } catch (err) { return e.json(404, { error: "Event or Profile not found" }); }
});
...
function parseSignedToken(token, requiredKeys) {
    ...
    if (!token || !rsvp) return e.json(400, { error: "Missing data" });
    const parts = parseSignedToken(token, ["e", "p", "s"]);
    if (!parts) return e.json(400, { error: "Invalid token" });
    const secret = getHmacSecret();
    const payload = `e=${parts.e}&p=${parts.p}`;
    if (!$security.equal(parts.s, $security.hs256(payload, secret))) return e.json(401, { error: "Invalid signature" });
    try {
        let roster;
        try { roster = $app.findFirstRecordByFilter("eventRosters", "event = {:e} && profile = {:p}", { e: parts.e, p: parts.p }); } 
        catch (err) { roster = new Record($app.findCollectionByNameOrId("eventRosters"), { event: parts.e, profile: parts.p, attendance: "Pending" }); }
        roster.set("rsvp", rsvp);
        $app.save(roster);
        return e.json(200, { success: true });
    } catch (err) { return e.json(500, { error: "Failed to update RSVP" }); }
});
...
function parseSignedToken(token, requiredKeys) {
    ...
    const { token } = e.requestInfo().body;
    if (!token) return e.json(400, { error: "Missing token" });
    const parts = parseSignedToken(token, ["p", "s"]);
    if (!parts) return e.json(400, { error: "Invalid token" });
    const secret = getHmacSecret();
    if (!$security.equal(parts.s, $security.hs256("p=" + parts.p, secret))) return e.json(401, { error: "Invalid signature" });
    try {
        const profile = $app.findRecordById("profiles", parts.p);
        profile.set("doNotEmail", true);
        $app.save(profile);
        return e.json(200, { success: true });
    } catch (err) { return e.json(404, { error: "Profile not found" }); }
});
...
function parseSignedToken(token, requiredKeys) {
    ...
    const authRecord = e.auth;
    if (!authRecord || authRecord.get("role") !== "admin") return e.json(403, { error: "Forbidden" });
    const { eventId } = e.requestInfo().body;
    if (!eventId) return e.json(400, { error: "Missing eventId" });
    const secret = getHmacSecret();
    if (!secret) return e.json(500, { error: "HMAC_SECRET not configured" });
    const payload = `e=${eventId}`;
    return e.json(200, { token: payload + "&s=" + $security.hs256(payload, secret) });
});
...
function parseSignedToken(token, requiredKeys) {
    ...
    if (token && sParam && !token.includes('s=')) { token = token + "&s=" + sParam; }
    if (!token) return e.json(400, { error: "Missing token" });
    const parts = {};
    token.split('&').forEach(p => { const kv = p.split('='); if (kv.length === 2) parts[kv[0]] = kv[1]; });
    if (!parts.e || !parts.s) return e.json(400, { error: "Invalid token" });
    const secret = getHmacSecret();
    const expected = $security.hs256("e=" + parts.e, secret);
    if (!$security.equal(parts.s, expected)) return e.json(401, { error: "Invalid signature" });
    try {
        const event = $app.findRecordById("events", parts.e);
        let setList = parseJsonField(event.get("setList")) || [];
        let pieces = [];
        try { pieces = $app.findRecordsByFilter("musicLibrary", "id != ''", "created", 1000).map(p => ({ id: p.id, parentId: p.get("parentId"), title: p.get("title"), composer: p.get("composer"), duration: p.get("duration"), audioTrackMapping: parseJsonField(p.get("audioTrackMapping")) || {}, collectionId: "pbc_music_library_001", collectionName: "musicLibrary" })); } catch (err) {}
        let voiceParts = [];
        try { const vp = $app.findFirstRecordByFilter("appSettings", "key = 'voiceParts'"); voiceParts = parseJsonField(vp.get("value")).voiceParts || []; } catch (e) {}
        return e.json(200, { event: { id: event.id, title: event.get("title"), date: event.get("date") }, setList, pieces, voiceParts });
    } catch (err) { return e.json(404, { error: "Not found" }); }
});
