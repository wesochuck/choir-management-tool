...
export default function EventRSVPView() {
  ...
  const createRsvpSessionMutation = useMutation({
    mutationFn: (data: {
      eventId: string;
      quantity: number;
      email: string;
      ...
      ticketService.createRsvpSession(
        data.eventId,
        data.quantity,
        data.email,
        data.name,
    ...
    onSuccess: (result) => {
      if (result.url) {
        ...
        setFormError('Success URL not returned');
      }
    },
    onError: (err: Error) => {
      setFormError(err.message || 'RSVP failed.');
    },
  });
      ...
      <div className="flex min-h-screen w-screen flex-col items-center justify-center p-4">
        <AppCard className="w-full max-w-[480px] text-center">
          <p className="text-danger-text m-0">
            {isError || !event ? 'Event not found.' : 'RSVP is closed for this event.'}
          </p>
  ...
  if (!event.isFreeRSVP) {
    return <Navigate to={`/tickets/${event.id}`} replace />;
  }

  const maxQuantity = event.maxPerRSVP || 2;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setFormError('');
    await createRsvpSessionMutation.mutateAsync({
      eventId: event.id,
      quantity,
      email: trimmedEmail,
      name: name.trim(),
      ...
      <AppCard className="w-full max-w-[720px]">
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-0.5">
            ...
            <h1 className="text-display m-0">RSVP</h1>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <Button
            type="submit"
            disabled={createRsvpSessionMutation.isPending || emailMismatch}
            className="h-12 w-full font-semibold"
            variant="primary"
          >
            {createRsvpSessionMutation.isPending ? 'Processing...' : 'Confirm RSVP'}
          </Button>
        </form>
      </AppCard>
    </PublicBrandingWrapper>
