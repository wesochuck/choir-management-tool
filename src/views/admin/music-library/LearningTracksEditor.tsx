...
export const LearningTracksEditor: React.FC<LearningTracksEditorProps> = ({
  ...
  return (
    <div className="border-border flex flex-col gap-1 rounded-md border bg-[rgb(0_0_0_/_2%)] p-2">
      {visibleKeys.map((partLabel) => {
        ...
        const displayName = isTutti ? (isMovement ? 'Tutti' : 'Tutti (Full)') : partLabel;
        ...
        return (
          <div
            key={partLabel}
            className={`flex flex-row items-center justify-between gap-4 transition-all duration-150 ${isMovement ? 'border-border scale-100 rounded-md border bg-[var(--bg-card-hover)] p-[6px_10px] text-[13px] shadow-none' : 'border-border scale-100 rounded-md border bg-[var(--bg-card-hover)] p-[8px_12px] shadow-none'}`}
            onDragOver={(e) => {
              e.preventDefault();
              e.stopPropagation();
            ...
            style={{
              padding: isDraggedOver ? (isMovement ? '5px 9px' : '7px 11px') : undefined,
          ...
          >
            <div className={`flex flex-col ${isMovement ? 'min-w-[80px]' : 'min-w-[90px]'}`}>
              <strong
                className={`text-[var(--text-color)] ${isMovement ? 'text-[12px]' : 'text-[13px]'}`}
              >
                {displayName}
              </strong>
