...
export default function TicketingView() {
  ...
  const [activeTab, setActiveTab] = useState<'willcall' | 'bundles' | 'orders' | 'share'>(
    'willcall'
  );
  ...
  const hasPurchases = editingBundle
          ...
          <AppCard noPadding>
            <div className="border-b border-slate-100 px-6 py-4">
              ...
              <p className="mt-1 text-sm font-medium text-slate-500">
                Choose a performance to view ticket sales, revenue, and will call activity. To
                enable ticketing for an event, go to the{' '}
                <Link
                  to="/admin/events"
            ...
            <div className="flex flex-col gap-6 p-6">
                        ...
                        <p className="mt-2 text-2xl font-black tracking-tight text-pink-600">
                          ...
                          {(
                            activePurchases.reduce(
                              (acc, p) => acc + p.unitPriceCents * p.quantity,
                              0
                            ) / 100
                          ).toFixed(2)}
                        </p>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="mt-2 text-2xl font-black tracking-tight text-amber-700">
                          $
                          {(activePurchases.reduce((acc, p) => acc + p.feeCents, 0) / 100).toFixed(
                            2
                          )}
                        </p>
                      </div>
              ...
              {showWarning && (
                <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-800">
                  ⚠️ Warning: Sold tickets ({totalTicketsSold}) have reached or exceeded 90% of
      ...
      {activeTab === 'share' && (
        <div className="animate-fade-in flex flex-col gap-6">
          <AppCard>
            ...
            <p className="mt-1 text-sm font-medium text-slate-500">
              Share these links or download high-quality QR codes for your flyers, concert programs,
              and social media.
            </p>
          </AppCard>
        ...
        <form id="bundle-form" onSubmit={handleSaveBundle} className="flex flex-col gap-4">
          ...
          <FormField label="Included Performances">
            {hasPurchases && (
              <div className="mb-2 rounded-lg border border-yellow-200 bg-yellow-50 p-2 text-xs text-yellow-700">
                ⚠️ This bundle has active purchases. Included events are locked to prevent data
                drift.
              </div>
            )}
          ...
          <FormField label="Public Details / Instructions">
            <textarea
              ...
              className="focus:border-primary focus:ring-primary block min-h-[100px] w-full resize-y rounded-lg border border-slate-200 bg-white p-3 text-sm text-slate-900 shadow-sm transition-colors outline-none placeholder:text-slate-400 focus:ring-1"
              value={publicDetails}
