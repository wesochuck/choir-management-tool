...
export default function PublicAuditionView() {
  ...
  const performanceQuery = useQuery({
    queryKey: performanceId
      ? queryKeys.auditions.performance(performanceId)
      : queryKeys.auditions.list,
    queryFn: async () => {
        ...
        {!isFormActive ? (
          <div className="flex flex-col gap-4 py-8 text-center">
            ...
            <p className="text-text-muted text-sm">
              Please check our social media or website for the next audition announcement.
            </p>
          </div>
        ) : submitted ? (
          <div className="flex flex-col gap-4 py-4">
            <div className="bg-success-bg text-success-text inline-flex items-center self-start rounded px-2 py-0.5 text-xs font-semibold tracking-wider uppercase">
              Request Sent
            </div>
            <p className="text-body m-0">{settings.confirmationMessage}</p>
            {homepageUrl && (
              <Button
                as="a"
                href={homepageUrl}
          ...
          <div className="flex flex-col gap-8">
            <div className="border-border bg-primary-light overflow-hidden rounded-xl border p-0 shadow-sm transition-all duration-200 hover:shadow-md">
              <button
              ...
              >
                <div className="flex flex-col gap-0.5">
                  ...
                  <span className="text-text-muted text-xs">
                    Please review the commitment for this concert series
                  </span>
                </div>
                <span
                  className={`text-[1.2rem] transition-transform duration-200 ${isScheduleExpanded ? 'rotate-180' : 'rotate-0'}`}
                >
                  ▼
                </span>
              </button>
              ...
              {isScheduleExpanded && (
                <div className="px-6 pb-6">
                  <div className="bg-surface flex flex-col gap-4 rounded-lg p-4">
                    <div>
                      <h3 className="text-primary-deep m-0">{targetPerformance.title}</h3>
                      <p className="m-0 text-sm">
                        Performance:{' '}
                        <strong>
                          {formatInTimezone(targetPerformance.date, timezone, {
                            weekday: 'long',
                            year: 'numeric',
                            month: 'long',
                            day: 'numeric',
                      ...
                      <div className="flex flex-col gap-1">
                        {rehearsals.length > 0 ? (
                          rehearsals.map((rehearsal) => (
                            <div
                              key={rehearsal.id}
                              className="border-border flex flex-col justify-between border-b py-1 text-xs md:flex-row"
                            >
                              <span>
                                {formatInTimezone(rehearsal.date, timezone, {
                                  month: 'short',
                                  day: 'numeric',
                                  weekday: 'short',
                                })}{' '}
                          ...
                          <p className="text-text-muted text-xs">Detailed schedule pending.</p>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-label">
                  Available Audition Times (Select all that apply)
                </label>
                <div className="border-border grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-3 rounded-lg border bg-neutral-100 p-4">
                  ...
                  {(!settings.slots || settings.slots.length === 0) && (
                    <span className="text-text-muted col-span-full py-3 text-center text-sm">
