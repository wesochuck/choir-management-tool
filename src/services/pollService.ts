...
export const pollService = {
  ...
  async getPolls(): Promise<RecordModel[]> {
    return await pb.collection('polls').getFullList({ sort: '-created' });
  },

  async createPoll(data: { question: string; archiveAt: string; eventId?: string }): Promise<RecordModel> {
    return await pb.collection('polls').create(data);
  },
  ...
  async getPollResponses(): Promise<RecordModel[]> {
    return await pb.collection('pollResponses').getFullList({
      expand: 'profileId',
      sort: '-updated',
    });
  },
