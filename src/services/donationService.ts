...
export const donationService = {
  async getDonationSettings(): Promise<DonationSettings> {
    try {
      const setting = await pb.collection('appSettings').getFirstListItem<AppSetting<DonationSettings>>(
  ...
  async saveDonationSettings(value: DonationSettings): Promise<AppSetting<DonationSettings>> {
    ...
    try {
      existing = await pb.collection('appSettings').getFirstListItem<AppSetting<DonationSettings>>(
    ...
    if (existing) {
      return await pb.collection('appSettings').update<AppSetting<DonationSettings>>(existing.id, payload);
    }
  ...
  async createDonationSession(data: {
    ...
    return await pb.send<{ url: string; sessionId: string }>('/api/checkout/create-donation-session', {
      method: 'POST',
      body: data
    });
  },
  ...
  async pollForDonationRecord(sessionId: string, retries = 5, delay = 1000): Promise<DonationRecord | null> {
    // @allow-sequential-await - Sequential polling checks for Stripe fulfillment status.
    for (let i = 0; i < retries; i++) {
      try {
        const record = await pb.collection('donations').getFirstListItem<DonationRecord>(
