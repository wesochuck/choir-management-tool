        ...
        <div className="flex-col" style={{ gap: 'var(--space-xs)' }}>
          ...
          {showSetList && setList && setList.length > 0 && (
            <>
              <button
                type="button"
                className="event-card-setlist-toggle"
                onClick={() => setIsSetListExpanded(value => !value)}
                aria-expanded={isSetListExpanded}
              >
                {isSetListExpanded ? 'Hide set list' : `Show set list (${setList.length})`}
              </button>
              
              <div className={`event-card-setlist flex-col ${isSetListExpanded ? 'expanded' : ''}`} style={{ marginTop: 'var(--space-sm)', backgroundColor: 'var(--bg)', padding: 'var(--space-md)', borderRadius: 'var(--radius-md)' }}>
