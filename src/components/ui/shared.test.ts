...
test('layoutOnly strips visual classes and keeps layout classes, including padding', () => {
  assert.equal(layoutOnly('w-full flex bg-red-500 p-2'), 'w-full flex p-2');
});
...
test('layoutOnly preserves margin classes (m-, mx-, my-, mt-, mb-, ml-, mr-)', () => {
  ...
  assert.equal(layoutOnly('m-2 mx-auto mt-4 bg-red-500'), 'm-2 mx-auto mt-4');
});
...
test('layoutOnly handles multi-class strings with arbitrary whitespace', () => {
  assert.equal(layoutOnly('  w-full   flex   bg-red-500  '), 'w-full flex');
});
