...
test('FormField renders label and children', () => {
  const { container } = render(
    React.createElement(FormField, { label: 'Name' }, React.createElement('input'))
  );
...
test('FormField renders required asterisk', () => {
  const { container } = render(
    React.createElement(FormField, { label: 'Name', required: true }, React.createElement('input'))
  );
...
test('FormField renders helpText', () => {
  const { container } = render(
    React.createElement(
      FormField,
      { label: 'Name', helpText: 'Enter your full name' },
      React.createElement('input')
    )
  );
...
test('FormField injects invalid into library Input child', () => {
  const { container } = render(
    React.createElement(FormField, { label: 'Name', error: 'Required' }, React.createElement(Input))
  );
  const input = container.querySelector('input');
