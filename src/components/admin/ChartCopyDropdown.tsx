  ...
  return (
    <div className="relative inline-flex" ref={containerRef}>
      <button
        ...
        className="border-border bg-surface text-text hover:border-primary hover:bg-primary-light focus:border-primary flex h-8 cursor-pointer items-center gap-1 rounded-md border px-2 text-xs transition-[border-color,box-shadow,background-color] duration-200 outline-none focus:shadow-[0_0_0_3px_rgba(74,124,89,0.25)]"
      ...
      >
        ...
        <svg
          className={`text-muted size-3.5 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={2.5}
      ...
      {isOpen && (
        <div
          className="animate-fade-in border-border bg-surface absolute top-full left-0 z-50 mt-1 max-h-[350px] min-w-[220px] overflow-y-auto rounded-lg border shadow-lg"
          role="listbox"
            ...
            groups.map((group) => {
              ...
              return (
                <div key={groupId}>
                  <div
                    id={`hdr-${groupId}`}
                    className="border-border bg-surface-muted text-muted border-b px-3 py-1.5 text-[11px] font-bold tracking-wider uppercase"
                  >
