...
export const BulkEventModal: React.FC<BulkEventModalProps> = ({
  ...
  const calculateRehearsalRange = (): { first: string; last: string } | null => {
    ...
    const firstUtc = zonedInputValueToUtc(
      `${firstYear}-${firstMonth}-${firstDay}T${time}`,
      timezone
    );
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      await dialog.showMessage({
        ...
        message:
          'Error generating rehearsals: ' + (err instanceof Error ? err.message : 'Unknown error'),
        variant: 'danger',
      });
    } finally {
      setIsLoading(false);
    ...
    >
      ...
      <form id="bulk-event-form" onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        {range && selectedPerformance && (
          <div className="bg-primary-light mt-1 flex flex-col gap-1 rounded-md border border-[rgba(74,124,89,0.2)] p-3 px-4 shadow-none">
            <div className="text-primary-deep flex items-center gap-2 text-sm font-semibold">
              <span className="text-lg">📅</span> Rehearsal Schedule Preview
            </div>
            <div className="text-text text-[0.8125rem] leading-relaxed">
              First Rehearsal:{' '}
              <strong className="text-primary-deep">
                {formatInTimezone(range.first, timezone, {
                  weekday: 'long',
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
            ...
            <div className="text-text text-[0.8125rem] leading-relaxed">
              Last Rehearsal:{' '}
              <strong className="text-primary-deep">
                {formatInTimezone(range.last, timezone, {
                  weekday: 'long',
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
            ...
            <div className="text-muted mt-1 text-xs">
              Generates {count} weekly rehearsals leading up to the performance on{' '}
              {formatInTimezone(selectedPerformance.date, timezone, {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
              })}
