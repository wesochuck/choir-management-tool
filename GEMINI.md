...
- **No Hardcoded Inline Styles:** Do not use hardcoded inline styles (`style={{ ... }}`) for layout, sizing, or micro-adjustments in React components. All styling must reside in external CSS stylesheets (e.g., `src/index.css`, `src/App.css`, or component-specific CSS files). Truly dynamic values (like drag coordinates or variable offsets) are permitted only if annotated with `// @allow-inline-style - [explanation]`.
