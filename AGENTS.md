...
- **Configure API Rules explicitly for client accessibility.** When creating collections programmatically, verify that client-accessible operations (such as `createRule`, `updateRule`, or `deleteRule`) allow access to the appropriate roles (e.g. `"@request.auth.role = 'admin'"`) rather than leaving them `null` (API-locked). Leaving them `null` causes client-side requests to fail with `403 Forbidden`, which triggers frontend session clearance and redirects administrative users to the login screen.
